...
.DEFAULT_GOAL = help
.PHONY: help build up down logs sh cc test test-db lint fix assets db migration migrate worker stripe install admin-password images audit
...
audit: ## Audit Lighthouse mobile sur un build de production local (Node.js et Chrome requis sur l'hôte)
	@bin/audit-lighthouse
